...
def test_with_valid_token(token):
    ...
    if verification_response.status_code == 200:
        ...
        print(json.dumps(verification_response.json(), indent=2) if verification_response.content else "No response body")
    ...
    if history_response.status_code == 200:
        ...
        print(json.dumps(history_response.json(), indent=2) if history_response.content else "No response body")
    ...
    if history_by_uuid_response.status_code == 200:
        ...
        print(json.dumps(history_by_uuid_response.json(), indent=2) if history_by_uuid_response.content else "No response body")

if __name__ == "__main__":
    # Use the provided token or fall back to default
    token = sys.argv[1] if len(sys.argv) > 1 else DEFAULT_TOKEN
    
    print(f"Testing API with token: {token[:5]}...{token[-5:] if len(token) > 10 else ''}")
