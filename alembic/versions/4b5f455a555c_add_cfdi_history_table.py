...
def upgrade():
    ...
    if not table_exists('api_tokens') or 'user_id' not in [c['name'] for c in inspect(op.get_bind()).get_columns('api_tokens')]:
    ...
    if not table_exists('cfdi_history'):
        ...
        op.create_index(op.f('ix_cfdi_history_id'), 'cfdi_history', ['id'], unique=False)
        op.create_index(op.f('ix_cfdi_history_uuid'), 'cfdi_history', ['uuid'], unique=False)
...
def downgrade():
    ...
    if table_exists('api_tokens') and 'user_id' in [c['name'] for c in inspect(op.get_bind()).get_columns('api_tokens')]:
        with op.batch_alter_table('api_tokens') as batch_op:
