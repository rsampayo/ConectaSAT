...
def test_get_cfdi_history_authenticated(client, db_session, override_get_token_dependency):
...
def test_get_cfdi_history_unauthorized(client, db_session):
    ...
    try:
        ...
        assert response.status_code == 403  # FastAPI's HTTPBearer raises HTTPException 403 by default
    finally:
