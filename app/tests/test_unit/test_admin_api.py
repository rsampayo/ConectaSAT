...
async def test_update_admin_password_endpoint(mock_db, mock_admin):
    ...
    password_data = SuperAdminUpdate(current_password="oldpassword123", new_password="newpassword123")
    ...
    with patch("app.api.admin.verify_password", return_value=True), \
         patch("app.api.admin.get_password_hash", return_value="new_hash"):
        result = await update_admin_password_endpoint(password_data, "testadmin", mock_db, mock_admin)
...
async def test_deactivate_admin_account_endpoint(mock_db, mock_admin):
    ...
    mock_db.query.return_value.filter.return_value.first.return_value = target_admin
    mock_db.query.return_value.filter.return_value.count.return_value = 2  # More than 1 active admin
