...
def test_sqlite_engine_creation():
    ...
    try:
        ...
        for args, kwargs in mock_create_engine.call_args_list:
            if args == expected_args and 'connect_args' in kwargs and kwargs['connect_args'] == expected_kwargs['connect_args']:
                found_expected_call = True
                break
        
        assert found_expected_call, "SQLite engine creation was not called with expected parameters"
