        ...
        if response.status_code == 200:
            ...
            try:
                ...
                for elem in root.findall('.//'):
                    ...
                    for attr_name in ['CodigoEstatus', 'EsCancelable', 'Estado', 'EstatusCancelacion', 'ValidacionEFOS']:
                        ...
                        if attr_name in elem.attrib:
                            ...
                            if attr_key == 'codigoestatus':
                                ...
                                result["estatus_cancelacion"] = elem.attrib[attr_name] if elem.attrib[attr_name] else "No disponible"
                ...
                if result["estado"] is None:
                    ...
                    for ns_prefix in ['a:', '']:
                        cancelable_elem = root.find(f'.//*{ns_prefix}EsCancelable', namespaces)
                        if cancelable_elem is not None and cancelable_elem.text:
                            result["es_cancelable"] = cancelable_elem.text
                            break
                            
                    for ns_prefix in ['a:', '']:
                        estatus_elem = root.find(f'.//*{ns_prefix}EstatusCancelacion', namespaces)
                        if estatus_elem is not None:
                            result["estatus_cancelacion"] = estatus_elem.text if estatus_elem.text else "No disponible"
                            break
                            
                    for ns_prefix in ['a:', '']:
                        codigo_elem = root.find(f'.//*{ns_prefix}CodigoEstatus', namespaces)
                        if codigo_elem is not None and codigo_elem.text:
                            result["codigo_estatus"] = codigo_elem.text
                            break
                            
                    for ns_prefix in ['a:', '']:
                        efos_elem = root.find(f'.//*{ns_prefix}ValidacionEFOS', namespaces)
                        if efos_elem is not None and efos_elem.text:
                            result["validacion_efos"] = efos_elem.text
                            break
                
                # Special handling for our test XML format
                if not root.findall('.//*a:Estado', namespaces) and not result["estado"]:
                    ...
                    if consulta_result is not None:
                        ...
                        if 'EsCancelable' in consulta_result.attrib:
                            result["es_cancelable"] = consulta_result.attrib['EsCancelable']
                        if 'EstatusCancelacion' in consulta_result.attrib:
                            result["estatus_cancelacion"] = consulta_result.attrib['EstatusCancelacion'] if consulta_result.attrib['EstatusCancelacion'] else "No disponible"
                        if 'CodigoEstatus' in consulta_result.attrib:
                            result["codigo_estatus"] = consulta_result.attrib['CodigoEstatus']
